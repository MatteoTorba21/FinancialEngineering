function [s, CI, P] = firstToDefault2Names(dates,discounts,TTM,R1,R2,datesCDS,intensities1,intensities2,rho,N)
...
% intensities1:     Vector of the exact intensities of the first obligor
% intensities2:     Vector of the exact intensities of the second obligor
...
for ii=1:length(result_matrix(:,1))
    ...
    if ContingentResetDate == 1
        accrualDelta = result_matrix(ii,1);
    else
        accrualDelta = result_matrix(ii,1) - sum(deltas(1:ContingentResetDate-1));
    end
...
end % function firstToDefault2Names