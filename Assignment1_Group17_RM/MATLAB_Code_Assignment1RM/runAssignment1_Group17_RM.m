...
% Students are free to design a bootstrap procedure based on the function PV_risky_bond_h

IG_h_curve = calibrate_h_curve(ZC_curve,IG_cf_schedule_1y, IG_Bond_dirty_price_1y,IG_cf_schedule_2y,IG_Bond_dirty_price_2y,R)

HY_h_curve = calibrate_h_curve(ZC_curve,HY_cf_schedule_1y, HY_Bond_dirty_price_1y,HY_cf_schedule_2y,HY_Bond_dirty_price_2y,R)

% First check: compare the numerical solution with the solution obtained by reversing the formula
IG_h_check = check_h(IG_cf_schedule_1y,IG_Bond_dirty_price_1y,IG_cf_schedule_2y,IG_Bond_dirty_price_2y,ZC_curve,R);
IG_error_numericalVSreversed = abs(max(max(IG_h_curve-IG_h_check)));
if IG_error_numericalVSreversed<1e-14
    disp('Numerical method and reversed equation give the same result for the IG h curve')
else
    fprintf('Error between numerical method and reversed equation for the IG h curve equal to: %.4f\n',IG_error_numericalVSreversed)
end

HY_h_check = check_h(HY_cf_schedule_1y,HY_Bond_dirty_price_1y,HY_cf_schedule_2y,HY_Bond_dirty_price_2y,ZC_curve,R);
HY_error_numericalVSreversed = max(max(HY_h_curve-HY_h_check));
if HY_error_numericalVSreversed<1e-14
    disp('Numerical method and reversed equation give the same result for the HY h curve')
else
    fprintf('Error between numerical method and reversed equation for the HY h curve equal to: %.4f\n',HY_error_numericalVSreversed)
end
...
% Plot of the hazard rate curves obtained:
figure()
xx = linspace(0,IG_h_curve(end,1),1000);
plot(xx,interp1(IG_h_curve(:,1),IG_h_curve(:,2),xx,'next','extrap'),'LineWidth',4);
hold on
plot(xx,interp1(HY_h_curve(:,1),HY_h_curve(:,2),xx,'next','extrap'),'LineWidth',4);
legend('IG','HY');
title('h curve','FontSize', 16);
xlabel('Time','FontSize', 16); ylabel('Hazard rate','FontSize', 16)
...
IG_z_1y = calibrate_z(ZC_curve,IG_cf_schedule_1y, IG_Bond_dirty_price_1y);
IG_z_2y = calibrate_z(ZC_curve,IG_cf_schedule_2y, IG_Bond_dirty_price_2y);
HY_z_1y = calibrate_z(ZC_curve,HY_cf_schedule_1y, HY_Bond_dirty_price_1y);
HY_z_2y = calibrate_z(ZC_curve,HY_cf_schedule_2y, HY_Bond_dirty_price_2y);
...
%      the formulas and the results in the .pdf report]
 Q = Transition_Matrix(IG_h_curve, HY_h_curve)